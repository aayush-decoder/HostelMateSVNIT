...
export default function BranchRoomMatrix({ selectedBranch="AI" }) {
  const [branchRooms, setBranchRooms] = useState({}); // {roomId: [users]}
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const token = localStorage.getItem("token");
    fetch(`http://hostelmate-nqe3.onrender.com/room_details/${selectedBranch}`, {
      ...
      .then((users) => {
        ...
        users.forEach((user) => {
          ...
          if (!groupedRooms[roomId]) groupedRooms[roomId] = [];
  ...
  const renderRoom = (roomId) => {
    const users = branchRooms[roomId];
    if (!users) return <div key={roomId} className="w-12 h-12 m-1 rounded-md bg-gray-800" />;

    const count = users.length;
    const bgClass = count === 2 ? COLOR_BRANCH.both : COLOR_BRANCH.one;
    const tooltip = users.map(u => `${u.name} (${u.admissionNumber})`).join(", ");

    return (
      <Tooltip key={roomId} title={tooltip} arrow>
        <div className={`w-12 h-12 m-1 rounded-md text-xs flex items-center justify-center text-white ${bgClass}`}>
          {roomId}
        </div>
      </Tooltip>
    );
  ...
  if (loading) return <div className="text-white">Loading matrix for branch: {selectedBranch}...</div>;

  return (
    <div className="bg-gray-900 p-4 rounded-xl text-white shadow-xl">
      <h1 className="text-2xl mb-6 font-semibold text-center">
        Branch Room Matrix for {selectedBranch}
      </h1>
      {renderFanWing()}
      {renderSquareWing()}
    </div>
